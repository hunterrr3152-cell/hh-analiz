...
async def cmd_analiz(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    st["state"] = "IDLE"
    msg = "🤖 <b>Dekont Analiz Paneli</b>\nLütfen yapmak istediğiniz işlemi seçin:"
    await update.message.reply_text(msg, parse_mode="HTML", reply_markup=get_menu_keyboard("IDLE"))

async def process_dekont_with_ai(image_bytes: bytes, chat_id: int) -> Tuple[bool, str]:
    st = chat_statements[chat_id]
    async with st["lock"]:
        if not st["unmatched_lines"]:
...
async def handle_callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if d == "cmd_upload_stmt":
        ...
        await query.edit_message_text("📂 <b>Hesap Hareketleri Bekleniyor...</b>\nLütfen PDF formatındaki hesap ekstresini gönderin.", parse_mode="HTML", reply_markup=get_menu_keyboard(st["state"]))
    elif d == "cmd_upload_dekont":
        if not st.get("iban") or st["iban"] == "Bilinmiyor":
            await query.edit_message_text("⚠️ <b>Önce Hesap Hareketleri Yüklemelisiniz!</b>", parse_mode="HTML", reply_markup=get_menu_keyboard("IDLE"))
            return
        st["state"] = "WAIT_DEKONT"
        await query.edit_message_text(f"🧾 <b>Dekont Bekleniyor...</b>\nKuyrukta: {len(st['queued_dekonts'])} dekont\nLütfen dekontları (fotoğraf veya PDF) gönderin.", parse_mode="HTML", reply_markup=get_menu_keyboard(st["state"]))
    elif d == "cmd_cancel":
        st["state"] = "IDLE"
        await query.edit_message_text("🤖 <b>İşlem İptal Edildi.</b> Ana Menü:", parse_mode="HTML", reply_markup=get_menu_keyboard("IDLE"))
    elif d == "cmd_reset":
        ...
        st["failed_list"] = []
        st["state"] = "IDLE"
        await query.edit_message_text("🛑 <b>Sistem Sıfırlandı!</b>\nKuyruk ve eşleşmeler temizlendi.", parse_mode="HTML", reply_markup=get_menu_keyboard("IDLE"))
    elif d == "cmd_stop":
        st["state"] = "IDLE"
        await query.edit_message_text("🛑 <b>Analiz Durduruldu!</b>", parse_mode="HTML", reply_markup=get_menu_keyboard("IDLE"))
    elif d == "cmd_analyze":
        if not st["queued_dekonts"]:
            await query.edit_message_text("⚠️ <b>Kuyrukta dekont yok!</b>\nLütfen önce dekont yükleyin.", parse_mode="HTML", reply_markup=get_menu_keyboard("IDLE"))
            return
        st["state"] = "ANALYZING"
        total = len(st["queued_dekonts"])
        msg = await context.bot.send_message(chat_id, f"⏳ <b>Analiz Başlıyor...</b> (0 / {total})\nLütfen bekleyin...", parse_mode="HTML", reply_markup=get_menu_keyboard("ANALYZING"))
        last_update = time.time()
        for i, file_bytes in enumerate(list(st["queued_dekonts"])):
            ...
            st["queued_dekonts"].remove(file_bytes)
            if time.time() - last_update > 2 or i == total - 1:
                prog_text = f"⏳ <b>Analiz Ediliyor...</b> ({st['processed_count']} / {total})\n✅ Başarılı: {st['matched_count']} | ❌ Başarısız: {st['failed_count']}"
                try:
                    await context.bot.edit_message_text(prog_text, chat_id=chat_id, message_id=msg.message_id, parse_mode="HTML", reply_markup=get_menu_keyboard("ANALYZING"))
                except Exception:
                    pass
                last_update = time.time()
        st["state"] = "IDLE"
        final_text = f"🎯 <b>Analiz Tamamlandı!</b>\n✅ Başarılı: {st['matched_count']}\n❌ Başarısız: {st['failed_count']}\n\n"
        if st["failed_list"]:
            final_text += "<b>Başarısız İşlem Detayları:</b>\n\n" + "\n\n".join(st["failed_list"])
        if len(final_text) > 4000:
            final_text = final_text[:4000]
        try:
            await context.bot.edit_message_text(final_text, chat_id=chat_id, message_id=msg.message_id, parse_mode="HTML", reply_markup=get_menu_keyboard("IDLE"))
        except Exception:
...
async def handle_files(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "WAIT_STATEMENT":
        ...
        if any(k in text.upper() for k in ["HESAP HAREKET", "EKSTRE", "HESAP OZETI", "HESAP ÖZETİ", "GUNCEL BAKIYE", "GÜNCEL BAKİYE"]):
            ...
            await update.message.reply_text(f"✅ <b>Hesap Hareketleri Yüklendi!</b>\n💳 IBAN: <code>{n_st['iban']}</code>\n📋 Toplam İşlem: {len(n_st['lines'])}", parse_mode="HTML", reply_markup=get_menu_keyboard("IDLE"))
